...
function! gnvim_lsp#hover#cursor_moved()
    ...
    if type(s:hover_pos) == type({})
        let start = s:hover_pos['start']
        let end = s:hover_pos['end']
...
function! s:timer_cb(id)
    call gnvim_lsp#hover#show_hover()
endfunction

function! gnvim_lsp#hover#abort()
    call timer_stop(s:hover_timer)
    let s:hover_pos = 0
    call gnvim#cursor_tooltip#hide()
endfunction
...
function! s:handle_hover(server, pos, screencol, screenrow, data) abort
    ...
    if !empty(a:data['response']['result']) && !empty(a:data['response']['result']['contents'])
        ...
        if has_key(a:data['response']['result'], 'range')
            let s:hover_pos = a:data['response']['result']['range']
            let l:pos[0] = s:hover_pos['start']['line']
            let l:pos[1] = s:hover_pos['start']['character']
        else
            let s:hover_pos = 0
        endif
